...
export = async function(fastify: FastifyInstance) {
    ...
    /** /api/file/all-bid-opening-record **/
    {
        const schema = {
        ...
        //@ts-ignore
		fastify.post('/file/all-bid-opening-record', {schema}, async (req, res) => {
            const {uid} = req.session.token!;

            
            let sid_list:{sid:RoskaSerials['sid']}[] = [], offset = 0;
            while(true) {
                ...
                //@ts-ignore
                const report_result = await Postgres.query(repost_list.join('\n')) as QueryResult<RoskaSerials & RoskaMembers & {name: User['name'], taiwan_date:string}>[];
